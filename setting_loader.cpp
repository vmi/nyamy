...
void SettingLoader::load_INCLUDE()
{
	...
	if (!loader.load(m_builder, (*getToken()).getString()))
...
void SettingLoader::load_DEFINE_MODIFIER()
{
	...
	while (!isEOL()) {
		t = getToken();
		Key *key =
			m_builder->searchKeyByNonAliasName(t->getString());
		if (!key)
...
void SettingLoader::load_DEFINE_ALIAS()
{
	...
	Token *t = getToken();
	Key *key = m_builder->searchKeyByNonAliasName(t->getString());
	if (!key)
		throw ErrorMessage() << _T("`") << *t << _T("': invalid key name.");
	m_builder->addAlias(name->getString(), key);
}
...
void SettingLoader::load_DEFINE_SUBSTITUTE()
{
	...
	for (AssignedKeys::iterator i = assignedKeys.begin();
		...
		m_builder->addSubstitute(*i, mkey);
}
...
void SettingLoader::load_DEFINE_OPTION()
{
	...
	if (*t == _T("KL-")) {
		...
		load_ARGUMENT(m_builder->correctKanaLockHandling());
		...
		load_ARGUMENT(m_builder->oneShotRepeatableDelay());
		...
		load_ARGUMENT(m_builder->sts4mayu());
		...
		load_ARGUMENT(m_builder->cts4mayu());
		...
		load_ARGUMENT(m_builder->mouseEvent());
		...
		load_ARGUMENT(m_builder->dragThreshold());
...
{
	Token *t = getToken();
	Key *key = m_builder->searchKey(t->getString());
	if (!key)
		throw ErrorMessage() << _T("`") << *t << _T("': invalid key name.");
	return key;
}
...
void SettingLoader::load_KEYMAP_DEFINITION(const Token *i_which)
{
	...
	if (doesLoadDefaultKeySeq) {
		...
		if (*t == _T(":")) {
			getToken();
			t = getToken();
			parentKeymap = m_builder->searchKeymapByName(t->getString());
			if (!parentKeymap)
				throw ErrorMessage() << _T("`") << *t
...
void SettingLoader::load_ARGUMENT(const Keymap **o_arg)
{
	...
	keymap = m_builder->searchKeymapByName(t->getString());
	if (!keymap)
...
void SettingLoader::load_ARGUMENT(const KeySeq **o_arg)
{
	...
	if (t->isOpenParen()) {
		...
		t = getToken();
		keySeq = m_builder->searchKeySeqByName(t->getString());
		if (!keySeq)
	...
	const tstringi &i_name, bool i_isInParen, Modifier::Type i_mode)
{
	...
	while (!isEOL()) {
		...
		if (t->isCloseParen() && i_isInParen)
			...
			getToken();
			t = getToken();
			KeySeq *ks = m_builder->searchKeySeqByName(t->getString());
			if (ks == NULL)
				throw ErrorMessage() << _T("`$") << *t
	...
	return m_builder->addKeySeq(keySeq);
}
...
{
	m_builder->addSymbol(getToken()->getString());
}
...
void SettingLoader::load_IF()
{
	...
	bool doesSymbolExist = m_builder->hasSymbol(t->getString());
...
void SettingLoader::load(const tstringi &i_filename)
{
	...
	tstring data;
	if (!m_configFiles.readFile(&data, m_currentFilename)) {
		Acquire a(m_soLog);
...
bool SettingLoader::load(SettingBuilder *i_builder, const tstringi &i_filename)
{
	m_builder = i_builder;
	m_isThereAnyError = false;
	...
	Symbols symbols;
	auto retry = []() -> bool {
		return DialogBox(g_hInst, MAKEINTRESOURCE(IDD_DIALOG_setting),
						 NULL, dlgSetting_dlgProc) != 0;
	};
	if (!m_configFiles.getFilename(i_filename, &path, &symbols, retry)) {
		if (i_filename.empty()) {
			Acquire a(m_soLog);
			m_configFiles.getFilename(i_filename, &path, nullptr, nullptr, 0);
			return false;
		} else
...
// load setting (public entry point)
std::unique_ptr<Setting> SettingLoader::load(
	const tstringi &i_filename, const Symbols &i_initialSymbols)
{
	SettingBuilder builder;
	for (const auto &s : i_initialSymbols)
		builder.addSymbol(s);
	if (!load(&builder, i_filename))
		return nullptr;
...
//=============================================================================
// CmdStreamLoader implementation (replaces BcInterpreter)
//=============================================================================
...
class CmdStreamLoaderImpl : public CmdLoadContext
{
private:
...
public:
	CmdStreamLoaderImpl(SettingBuilder &builder, SyncObject *soLog, tostream *log)
...
std::unique_ptr<Setting> SettingLoader::loadFromStream(std::istream &in,
													   const Symbols &i_initialSymbols)
{
	...
	// Run interpreter
	CmdStreamLoaderImpl impl(builder, m_soLog, m_log);
	impl.load(in);
