...
# Generate scripter/mruby-dist/mruby.props.
#
# mruby hands its build-time configuration to consumers through compiler -D
# flags only: mrbconf.h is a plain source header holding defaults and legacy
# aliases, and the build never rewrites it.  Several public headers change
# shape with those defines (MRB_USE_TASK_SCHEDULER appends a member to struct
# mrb_state, MRB_UTF8_STRING redefines RSTR_SINGLE_BYTE_P, ...), so every
# project linking mruby.lib must compile with the same set.  The generated
# property sheet is imported by the nyamy-scripter projects, which keeps the
# two sides in sync when this config changes.
#
# The flags are read back from libmruby.flags.mak, which the mruby build emits
# for exactly this purpose (it is also what mruby-config --cflags reports).

# Extracts the consumer-relevant parts of MRUBY_CXXFLAGS.
function Get-MrubyFlags {
	param([string]$MakPath)

	$line = Get-Content $MakPath | Where-Object { $_ -match '^MRUBY_CXXFLAGS\s*=' } | Select-Object -First 1
	if (-not $line) { Write-Error "MRUBY_CXXFLAGS not found in: $MakPath" }
	$flags = ($line -split '=', 2)[1]

	# mruby's own knobs only.  Toolchain noise such as /D_CRT_SECURE_NO_WARNINGS
	# is not part of the ABI contract and must not leak into the consumers.
	$defines = [regex]::Matches($flags, '[/-]D\s*"?((?:MRB_|HAVE_MRUBY_)\w+)') |
		ForEach-Object { $_.Groups[1].Value } |
		Select-Object -Unique | Sort-Object

	# /EHs (SyncCThrow) vs /EHsc (Sync) vs /EHa (Async).  This must match the
	# library: the 'c' in /EHsc lets the compiler assume extern "C" functions
	# never throw, and the whole mruby API is extern "C" -- with C++ exceptions
	# enabled, a raise unwinding out of mrb_* would then skip destructors.
	$eh = $null
	$m = [regex]::Matches($flags, '[/-]EH(a|sc|s)(?![a-z])')
	if ($m.Count -gt 0) {
		switch ($m[$m.Count - 1].Groups[1].Value) {
			'a'  { $eh = 'Async' }
			'sc' { $eh = 'Sync' }
			's'  { $eh = 'SyncCThrow' }
		}
	...
	return @{ Defines = $defines; ExceptionHandling = $eh }
}

# All configs share one property sheet, so verify they agree before writing it.
$flags = $null
foreach ($cfg in $Config) {
	$mak = Join-Path $buildDir "$cfg\lib\libmruby.flags.mak"
	if (-not (Test-Path $mak)) { Write-Error "Not found: $mak" }
	$cur = Get-MrubyFlags $mak
	if (-not $flags) { $flags = $cur; continue }
	if (($cur.Defines -join ';') -ne ($flags.Defines -join ';') -or
	    $cur.ExceptionHandling -ne $flags.ExceptionHandling) {
		Write-Warning (
			"mruby build flags differ between configs; mruby.props is generated from " +
			"$($Config[0]) only.  Align the configs in tools/mruby_build_config.rb."
		)
	}
}

$defineList = ($flags.Defines -join ';') + ';%(PreprocessorDefinitions)'
$ehElement  = if ($flags.ExceptionHandling) {
	"`r`n      <ExceptionHandling>$($flags.ExceptionHandling)</ExceptionHandling>"
} else { '' }

$propsPath = Join-Path $distDir 'mruby.props'
$propsBody = @"
<?xml version="1.0" encoding="utf-8"?>
<!--
  mruby.props - GENERATED FILE, DO NOT EDIT.

  Regenerated by tools/build_mruby.ps1 from
  scripter/mruby-build/$($Config[0])/lib/libmruby.flags.mak.

  mruby passes its build-time configuration to consumers as compiler -D flags
  only; mrbconf.h is never rewritten by the build.  Public headers change shape
  with these defines, so every project linking mruby.lib must compile with the
  same set. Import this sheet instead of restating them.

  /std:c++20 is deliberately not propagated: mruby needs it only because
  enable_cxx_exception compiles its own C sources as C++.  Consumers merely
  include the headers and keep the project's own C++ standard.
-->
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <ItemDefinitionGroup>
    <ClCompile>
      <PreprocessorDefinitions>$defineList</PreprocessorDefinitions>$ehElement
    </ClCompile>
  </ItemDefinitionGroup>
</Project>
"@

Set-Content -Path $propsPath -Value $propsBody -Encoding utf8
Write-Host "Generated: scripter/mruby-dist/mruby.props"
