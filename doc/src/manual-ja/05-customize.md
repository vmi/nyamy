...
個々のウィンドウの`⟨ウィンドウクラス名⟩`と`⟨ウィンドウタイトル名⟩`を調べるには、タスクトレイメニュー[調査(I)...](#menu-i)の「ウィンドウの調査」、または`⟨FUNCTION⟩` [`&WindowIdentify`](#function_WindowIdentify) を利用してください。前者は**カーソルの下**のウィンドウを、後者は**フォーカスを持っている**ウィンドウを報告するので、両者の結果は必ずしも一致しません。`window` が照合するのはフォーカスを持っているウィンドウなので、食い違った場合は [`&WindowIdentify`](#function_WindowIdentify) の出力に合わせてください。
