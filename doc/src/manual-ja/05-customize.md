...
<span class="menu-item">ログ</span>ダイアログの編集欄が保持するテキスト量の上限は、既定で 20000 文字です。`nyamy.ini` の `logMaxSize` で変更できます。

```ini
logMaxSize=50000
```

未指定・0 以下・数値として解釈できない値の場合は既定値 (20000) が使われます。上限そのものに制限はありませんが、値を大きくするほど追記のたびのコストが増える点に注意してください。この設定は NYamy の起動時に読み込まれます (タスクトレイメニューの<span class="menu-item">Reload</span>では反映されません)。

切り詰めは行単位ですが、上限を超えるたびに 1 行ずつ削るのではなく、**古い方から約 3 分の 2 をまとめて捨てます**。編集欄からのテキスト削除は保持量に比例して重くなるため、上限に達してから 1 行ごとに削ると、上限を大きくした分だけログ 1 行あたりのコストが増えてしまうからです。このため編集欄が実際に保持している量は上限そのものではなく、**上限の約 3 分の 1 から上限までの間で変動します**。同じ量のログを流しても残っている先頭行が毎回変わるのは、この動作によるものです。
