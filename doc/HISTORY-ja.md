...
:   モディファイヤ
    ...
    :   - WindowsXP の「ユーザーの切り替え」機能に対応。
        ...
        - [`109.mayu`](../109.mayu) に MultiMedia Keyboard 用の特殊キーなどを追加。
...
:   - Windows XP 用のドライバ。(Thanks to 小林)
    ...
    - [`contrib/109onAX.mayu`](../contrib/109onAX.mayu) を追加。(Thanks to 松本博紀)

    - [`104.mayu`](../104.mayu) と [`contrib/ax.mayu`](../contrib/ax.mayu) の記述ミスを修正。(Thanks to 松本博紀)

    - [`contrib/DVORAKon109.mayu`](../contrib/DVORAKon109.mayu) を追加。(Thanks to 2ch)
...
:   - One Shot モディファイヤの例が間違っていたので修正。(Thanks to 小林)
    ...
    - [`mayu-mode.el`](../mayu-mode.el) で `M-;` (`indent-for-comment`) が使用できるようになった (Thanks to 永野圭一郎)
...
:   - [`default.mayu`](../default.mayu) で、[[英数]] を [[Control]] として使用している場合に `C-Q` が正しく働かない問題を修正。version 3.19 で修正したものと原因は同じ。(Thanks to hanawa)
    ...
    - `R-` を新設。(Thanks to hanawa)。[`mayu-mode.el`](../mayu-mode.el) で色がつくようにした。
...
:   - [`104on109.mayu`](../104on109.mayu) で「\`」が入力できなかったバグを直した。(Thanks to Oiwa)

    - One Shot (キーリピート有) を追加。

    - `⟨FUNCTION⟩` `&Toggle` で、強制的にオンオフできるようにした。

    - [`109.mayu`](../109.mayu) に VAIO タワー用の特殊キーを追加。
...
:   - 代用定義を追加した。おかげで [`104on109.mayu`](../104on109.mayu) などがスマートに定義できるようになった。

    - [`mayu-mode.el`](../mayu-mode.el) で代用定義にも色がつくようにした。
...
:   - Windows95 で CPU 時間を 100% 占めてしまう問題がなおった。(Thanks to 小林義明)
    ...
    - [`contrib/98x1.mayu`](../contrib/98x1.mayu) を追加。(Thanks to HAJANO Naòqui)
...
:   - このバージョンは内部構造を大幅に変えたので不安定である可能性が高いです。どうしてもこのバージョンが必要な人のみ使用してください。
    ...
    - [`default.mayu`](../default.mayu) を使っている場合は、`Global` キーマップで [[英数]] を [[Control]] にしているが、`Global` キーマップを親キーマップとして持たない `keymap2` の中では (当然ながら) [[英数]] を [[Control]] として使用できないので、ちゃんと `Global` キーマップを親キーマップとして持つように設定ファイルを修正。

    - [`default.mayu`](../default.mayu) で、`-DMAP-ESCAPE-TO-META` とすると [[Alt]] + [[X]] などを [[ESCAPE] [X]] で代用するようにした。
    ...
    - [`contrib/ax.mayu`](../contrib/ax.mayu) を追加。(Thanks to KAWABE Nobukazu)
    ...
    - [`default.mayu`](../default.mayu) を使用している場合、シンボル `ZXCV` を定義しておくと、[[Control]] + [[Z]], [[Control]] + [[X]], [[Control]] + [[C]], [[Control]] + [[V]] が Windows と同じ動きをするようになります。(Thanks to HANAWA Yoshio)

2000/12/17 version 3.18

:   - ヘルプが開けないバグの修正。

    - [`mayu-mode.el`](../mayu-mode.el) が展開されていなかったのを直した。ついでに少しだけ修正。

    - `IC-` が時々無視されるバグが直ったはず。

    - `⟨FUNCTION⟩` `&HelpMessage` を追加。[`default.mayu`](../default.mayu) を使っている場合はメモ帳で [[Control]] + [[X]] を押してみよう (ただし IE5.0 以上を使っている場合のみ)。
...
:   - メーリングリストを [Source Forge](http://lists.sourceforge.net/mailman/listinfo/mayu-support) へ移動。
    ...
    - Emacs で設定ファイルを編集する人のために [`mayu-mode.el`](../mayu-mode.el) を追加。ただし、色がつくだけです。

    - `⟨FUNCTION⟩` `&WindowIdentify` で、ウィンドウの位置と大きさも表示するようにした。

    - [`contrib/keitai.mayu`](../contrib/keitai.mayu) を追加。テンキーで携帯電話式ひらがな入力をしようという画期的設定ファイル (注:Joke)。(Thanks to HANAWA Yoshio)
...
:   - [`contrib/dvorak.mayu`](../contrib/dvorak.mayu) を更新。(Thanks to KANAI Makoto)

    - [`contrib/mayu-settings.txt`](../contrib/mayu-settings.txt) を更新。(Thanks to HANAWA Yoshio)
...
:   - [`contrib/dvorak.mayu`](../contrib/dvorak.mayu) を追加。(Thanks to KANAI Makoto)
...
:   - `⟨FUNCTION⟩` `&WindowHMaximize` を追加。
    ...
    - [`contrib/mayu-settings.txt`](../contrib/mayu-settings.txt) の追加。(Thanks to HANAWA Yoshio)
...
:   - [`104on109.mayu`](../104on109.mayu) と [`109on104.mayu`](../109on104.mayu) を作った。
